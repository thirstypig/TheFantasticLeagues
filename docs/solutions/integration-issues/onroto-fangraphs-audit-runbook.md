...
| 2026-08-03 | 08.02.26 | 72/80 exact — all 8 mismatches on ONE team (Demolition); other 7 cell-for-cell. **ROOT CAUSE OPEN.** The IL-exclusion explanation below fit the arithmetic but was **FALSIFIED the same day**: six of the seven *non-diverging* teams also have IL-at-period-start players with real stats (Chourio, Ramos, Palencia, Priester, Díaz, Lindor) and show zero divergence. If the mechanism were real they would diverge too. Pitching also runs the opposite direction (FBST +4 K, worse ERA/WHIP), which IL exclusion cannot produce. Superseded detail — Acuña IL-stashed 07-05→08-02 (== all of Period 5), returned to play 07-27, 6 games. Four-way **verified** (the measurements stand, the causal claim does not): `reconcilePeriodStats(39)` = 219/219 exact vs MLB, and FBST == statsapi == Baseball Reference on Acuña (R5/HR2/RBI2/SB0/AB22). Adding IL windows back closes HR exactly; **residual −1 R / −2 RBI / −2 SB + pitching in the opposite direction (K +4, ERA +.04) remains OPEN** — needs a per-player FG *period* slice (FG per-team pages are season-YTD only). See [[il-stashed-player-returns-to-play-creates-phantom-fangraphs-delta]]. |
...
| 2026-08-30 | 08.29.26 | **All 6 closed periods PASS vs MLB statsapi** (1,269 player-period checks, 0 mismatches, 0 coverage gaps) — stored data is correct everywhere, so every residual is *attribution*, not bad data. Season leg FINDINGS on 5/8 teams. **Dominant cause: roster moves recorded late relative to OnRoto.** Drops entered in FBST with `effDate = 2026-08-30` for players OnRoto had already removed, so FBST credits a full extra Period 6. Exact to the stat line: The Show = Peralta (W1 K20) + Hicks (R8 HR3 RBI12) + García (R12 HR5 RBI12) = its entire residual across all 7 categories; Devil Dawgs = Erik Miller (W1 K18), 100%; RGing Sluggers = Heliot Ramos (R1 RBI1 SB1), 100%; Los Doyers = Pallante + Wrobleski (W1 K24) of W3 K27. **Falsification note — the naive rule is wrong:** "dropped today ⇒ over-credited" is FALSIFIED by Skunk Dogs, which made 6 moves the same day (incl. drops with real P6 stats) and reconciles exactly. The mechanism requires the FBST drop to be late *relative to OnRoto*, not merely recorded today. **Still OPEN:** confirming real drop dates from OnRoto's transaction log; Los Doyers' residual W2 K3; and Demolition's *negative* pitching residual (W−2 SV−4 K−4), which is a different, unidentified mechanism. Also fixed the instrument this run (PR #435): the season leg summed the ACTIVE period, and Period 7 opened that morning with zero PSP rows → 188 spurious skips and a false INCOMPLETE. Post-fix: skipped 0. |
| 2026-08-31 | — | **Closed out every residual from the 08-30 run, and fixed the last instrument bug (todo #308).** All three items left OPEN above are now resolved: Los Doyers = **Foster Griffin** (W3 K23, exactly his P6 line, dropped in FBST 08-30) + **Paul Sewald** (K4) — the earlier Pallante + Wrobleski attribution was **WRONG**, both match OnRoto exactly; Demolition's *negative* pitching = **Jacob Webb**, on OnRoto's roster with no FBST roster row at all (a missing ADD: −W2 −SV4 −K9), partly offset by **Emmet Sheehan** (+K4). statsapi and Baseball Reference agree cell-for-cell on Sheehan, Webb and Sewald, so **no residual is a stat-data error — every one is roster state.** Sewald's K+4 is the lone unproven case (W and SV match exactly, only K differs — the FG-stale signature, unprovable because FG has no per-period slice). **Third mechanism, instrument-only, now FIXED:** `isInPeriodWindow` was binary per period and `counted` was keyed `teamId:playerId`, so a player traded mid-period earned his WHOLE period line from BOTH teams (confirmed on Trade 22: Diamond Kings' entire residual was exactly Teoscar Hernández + Braxton Ashcraft's P7 lines). The accumulator now routes mid-period players through **clamped daily stats**, mirroring `standingsService`'s hybrid (todo #286) — `findMidPeriodPlayers` is the single shared definition so `findCoverageGaps` agrees with it. **Before/after prod differential across all 6 closed periods: no movement**, because the only closed-period mid-period players are two with zero stats and one whose stints are all the same team; Trade 22 sits in Period 7, still ACTIVE and therefore outside the completed-only season leg. Verified Aaron Ashby's daily rows reconstruct his PSP exactly (K9 IP9.33 ER3 BB_H13), so clamping loses nothing. **Data anomaly surfaced, not yet fixed:** Chase Dollander has a Roster row with `acquiredAt 2026-06-03` and `releasedAt 2026-05-17` — released before acquired. |
