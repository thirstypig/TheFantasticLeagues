...
- **[Testing Strategy](docs/guides/testing-strategy.md)** — Unit/integration tests, configuration, 2633 tests across 36 modules
...
2633 app tests (1664 server main suite + 7 integration [4 draft + 3 IL-fee] in the separate `db-integration` CI job + 962 client) plus 133 MCP tests (83 fbst-app + 50 mlb-data, run separately). The server main suite now also runs the docs-system script tests (`scripts/__tests__/*.test.mjs`, 41 tests) via a `../scripts/**` include. Unit/integration by feature module, configuration, how to run tests. See **[Testing Strategy guide](docs/guides/testing-strategy.md)**.
...
`/docs` is a browsable internal knowledge base rendered by the in-app board at `/docs`.
The board reads each file's **metadata, never its filename**. Full spec:
**[docs/README-DOCS.md](docs/README-DOCS.md)** (DOC-001).

### Frontmatter — required on every authored doc

A file without this block still exists on disk but **cannot be indexed, filtered, or
cross-linked**.

```yaml
---
id: PRD-001                     # stable forever — never reused or renumbered
title: "Human-readable title"   # what the sidebar shows
description: "One line, no markdown."
type: prd                       # see README-DOCS §4
status: draft                   # draft | active | locked | done | deprecated
phase: null                     # e.g. "Phase 3.5", or null
owner: james
tags: []                        # controlled vocabulary ONLY
links: []                       # other doc ids — this is the traceability
updated: YYYY-MM-DD
---
```

`status` describes **the document**. PRDs carry an extra optional
`feature_status: shipped | in-progress | planned | abandoned` describing **the feature** —
a current, trustworthy PRD about an unbuilt feature is a real thing.

### ID scheme

`PRD-###` (one file each, `docs/product/prds/`) · `ADR-###` (one file each,
`docs/engineering/adrs/`) · `DOC-###` (everything else authored) · `RISK-###` and `EXP-###`
(row ids inside the risks register / experiment log, not files).

⚠️ **New ADR files start at ADR-015.** `ADR-013` and `ADR-014` are already cited in this
file as real decisions whose files were never written.

### Controlled tag vocabulary — 15 tags, this is the whole list

`scoring` · `roster` · `players` · `transactions` · `draft` · `league-admin` · `data-sync` ·
`multi-sport` · `ai` · `auth` · `database` · `deploy` · `testing` · `design-system` ·
`docs-system`

**Never invent a tag at write time.** Adding one means editing README-DOCS §3 first and
logging it in the amendment block there. Freeform tags are how a tag system decays into a
second, worse search box. `bug` / `urgent` / `wip` are `status`, not tags.

### Generated docs — never hand-edit

| File | Generated by |
|---|---|
| `docs/under-the-hood/stats.md`, `costs.md`, `system-status.md` | `npm run docs:refresh` |
| The `<!-- DOCS:STATUS -->` block in `README.md` + `CLAUDE.md` | `npm run docs:refresh` |
| `docs/INBOX.md` | `node scripts/sync-inbox.mjs` |

Edit the **inputs** instead: `docs/costs.config.json` for costs, `docs/_comments.json` for
the inbox.

**Run `npm run docs:refresh` before every push.** `--check` exits 1 if anything is stale,
so it can be wired to CI.

### The comment-inbox ritual

**At the start of a session, read [`docs/INBOX.md`](docs/INBOX.md).**
...
3. Write a resolution — set `status: resolved` in `docs/_comments.json` with a note **and a
   link** (commit SHA, PR number, or doc id), then regenerate. The link is enforced: the
   script fails without it. "I fixed it" with no link is not a resolution.

### Pointer docs — do not fork these

`docs/product/roadmap.md` → `server/data/planning.json` · `docs/product/todos.md` →
`todos/` · `docs/under-the-hood/changelog.md` → `docs/changelog.md`.
Their real sources are already wired into the live app (PR #423, #424). Forking them
re-creates the exact problem those PRs fixed.

### Honesty conventions

Retroactive docs tag every claim `[intended]` / `[inferred]` / `[unknown]`. **A doc full of
`[unknown]` flags is a success** — those are the questions worth surfacing. Never fabricate
a KPI, metric, or intent to make a document look complete. Unverified numbers in
`costs.md` stay flagged `VERIFY`.

## Coding Guidelines
- **Feature module boundaries (ADR-015)**: feature modules must not import from other feature modules — promote shared code to `client/src/components/shared`, `client/src/hooks`, `client/src/lib`, `server/src/lib`, or `shared/api`. The existing 97 violations are grandfathered in `scripts/feature-isolation-baseline.json`; **new ones must not be added**. Check with `node scripts/check-feature-isolation.mjs` (`--report` for the full graph). Duplicating a small helper is preferable to a bad coupling.
