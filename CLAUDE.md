...
<!-- now-tldr -->
Fantasy baseball for the dozen-owner, auction-draft, keeper-league crowd that Yahoo and ESPN never really served. App is live for OGBA, the auction has wrapped, and the season is in flight. Roster-rules enforcement landed and is on for OGBA. **Current focus: the Aurora design system rollout from `claude.ai/design`'s `Aurora System.html` handoff.** Site-wide chrome (floating Topbar + bottom Dock) replaced the legacy AppShell sidebar in PR #147; 21 pages now have Aurora hero treatment (Home, Standings, Team, Players, PlayerDetail, Matchup, Weekly Report, AuctionValues, AuctionResults, Activity, Commissioner, Admin/Dashboard/Users, Todo, Board, Teams, AI Hub, Draft Report, Keeper Selection, Rules); the 3 auction post-draft children (DraftReport / BidHistoryChart / AuctionReplay) got deep Aurora ports in PR #151. **What's left:** PR-3 auction live floor (dedicated session per rollout plan — WS bid storm + 200ms timer); ~14 lower-traffic info pages (Payouts/Archive/Profile/Tech/Roadmap/etc.) which already inherit the shell but lack the Glass(strong) hero; and the pre-auth pages (Login/Signup) which intentionally stay outside AuroraShell.
<!-- /now-tldr -->
