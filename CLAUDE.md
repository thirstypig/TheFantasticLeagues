...
<!-- now-tldr -->
Fantasy baseball for the dozen-owner, auction-draft, keeper-league crowd that Yahoo and ESPN never really served. App is live for OGBA, the auction has wrapped, and the season is in flight. **Current focus is OGBA in-season correctness and roster-management UX**: Roster Hub is the active Yahoo-style direction, add/drop + IL stash + IL activate use server-backed previews, and confirm buttons unlock only when the full-roster matcher accepts the move. Home is now the daily league cockpit: My Team Today, Weekly Insights, current standings, league activity with full fantasy team names, pending trade proposals, Around the League, League Board, and Injured List. **Planning source of truth is `server/data/planning.json`**: micro todos live in `categories[].tasks`, macro roadmap items live in `roadmap[]`; do not recreate `TODO.md`, `server/data/todo-tasks.json`, or `docs/ROADMAP.md`. SEO/blog work is deferred while roster management, standings accuracy, daily stats, and dashboard UX stabilize. Display rules: frontend league UI never shows fantasy team codes; OGBA slots are `P`, `OF`, `CM`, `MI`, `DH`. Recent focused verification: Home/AddDrop targeted tests passing, broader 50-test client slice passing, client `tsc --noEmit` passing, and Home browser smoke clean on desktop/mobile. Style sheet at `docs/aurora-design-system.md`; active status doc at `docs/CURRENT_STATUS.md`.
<!-- /now-tldr -->
...
### Current Test Coverage (961 server + 583 client + 50 MCP + 1 E2E = 1595 tests, 28 feature modules)
