...
export default function AIHub() {
  ...
  const features: AIFeature[] = [
    ...
    {
      id: "draft-report-card",
      title: "Draft Report Card",
      description: "Auction-day values & busts per team at the 1/3, 2/3, and end-of-season checkpoints — anchored to actual stats.",
      icon: Trophy,
      category: "season",
      available: hasRosterData,
      lockReason: "Available after rosters are drafted",
      navigateTo: "/draft-report-card",
      scope: "league",
      trigger: "Computed on demand against the latest available stats. Three fixed checkpoints — 2/3 and Final unlock as the season progresses; the active checkpoint runs in PREVIEW mode.",
      model: "Pure computation — no LLM. Surplus = composite z-score across 5 roto cats minus standardized log(auction price).",
      promptSummary: "For each team, intersects auction-day rosters with the current active roster, then ranks by surplus. Top 3 surplus = values, bottom 3 = busts. Hitters and pitchers z-scored within their own pools (ERA/WHIP sign-flipped) but compete head-to-head for the slots.",
      dataUsed: [
        "Auction-day rosters (frozen at the auction close)",
        "Current active rosters (drops/trades excluded)",
        "Period stats summed through the selected checkpoint",
        "Min-sample floor: AB>=30 for hitters, IP>=10 for pitchers",
      ],
    },
    {
      id: "bid-advice",
